...
class Model(object):
    def __init__(self, x, y, l, window, opt, lr, init_emb, dim_emb, dim_hidden, n_vocab, L2_reg, unit,
        ...
        if sim == 'cos':
            ...
            y_score = sigmoid(T.batched_dot(T.dot(h_1, self.W_out), h_2.dimshuffle(0, 2, 1)))
        y_score_hat = T.max(y_score, 1)

        """ Objective Function """
        if sim == 'cos':
            self.nll = mean_squared_loss(y_score_hat, y)
        else:
            self.nll = binary_crass_entropy(y_score_hat, y)
        ...
        """ Predicts """
        y_binary = T.switch(y_score_hat > 0.5, 1, 0)  # 1D: batch
        y_hat = T.argmax(y_score, 1)

        """ Check Accuracies """
        self.correct = T.eq(y_binary, y)
        self.correct_r = T.eq(y_hat, y)
